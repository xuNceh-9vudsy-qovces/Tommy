...
public class MonthChartActivity extends AppCompatActivity {
    ...
    private void initFrag() {
        ...
        //將Fragment加載到Activity當中
    }

    //統計某年某月的收支情況數據
    private void initStatistics(int year, int month) {
        float inMoneyOneMonth = DBManager.getSumMoneyOneMonth(year, month, 1);  //收入總錢數
        float outMoneyOneMonth = DBManager.getSumMoneyOneMonth(year, month, 0);  //支出總錢數
        int incountItemOneMonth = DBManager.getCountItemOneMonth(year, month, 1);  //收入多少筆
        int outcountItemOneMonth = DBManager.getCountItemOneMonth(year, month, 0);  //支出多少筆
